...
async function readYaml(filePath, label) {
  try {
    ...
    fail("INVALID_GRAPH_CONFIG", `${label} could not be read as YAML: ${error.message}`);
  }
}
...
function validateVocabulary(vocabulary) {
  ...
  if (vocabulary.version !== 1) fail("INVALID_GRAPH_CONFIG","visual vocabulary must use version 1");
  ...
  if (
    ...
    fail("INVALID_GRAPH_CONFIG","visual vocabulary must define exactly the five approved visual types");
  }
  for (const [type, entry] of Object.entries(vocabulary.visual_types)) {
    ...
    if (
      ...
      fail("INVALID_GRAPH_CONFIG",`visual_types.${type} has invalid roles or element limits`);
    }
  }
  for (const field of ["icon_ids", "pose_ids", "symbol_ids"]) {
    if (
      ...
      fail("INVALID_GRAPH_CONFIG",`${field} must be a non-empty unique list`);
    }
  }
}
...
export async function loadGraphRuntime(projectRoot, graphRoot) {
  ...
  if (runtimeConfig.version !== 1) fail("INVALID_GRAPH_CONFIG","graph runtime config must use version 1");
  exactKeys(
  ...
  for (const [key, value] of Object.entries(runtimeConfig.limits)) {
    if (!Number.isInteger(value) || value < 1) fail("INVALID_GRAPH_CONFIG",`runtime limit ${key} must be a positive integer`);
  }

  const sharedRoot = await realpath(path.join(projectRoot, runtimeConfig.shared_visual_skill_root));
  if (!within(sharedRoot, projectRoot)) fail("INVALID_GRAPH_CONFIG","shared visual skill root escapes the project");
  ...
  if (!within(overlayPath, graphRoot) || !within(vocabularyPath, graphRoot)) {
    fail("INVALID_GRAPH_CONFIG","graph profile or vocabulary escapes the graph skill");
  }
  ...
  if (profileOverlay.version !== 1 || profileOverlay.id !== "lux-whiteboard-paragraph") {
    fail("INVALID_GRAPH_CONFIG","paragraph profile overlay must be lux-whiteboard-paragraph version 1");
  }
  ...
  if (
    ...
    fail("INVALID_GRAPH_CONFIG","paragraph profile overlay must extend the configured shared base profile");
  }
  if (
    ...
    fail("INVALID_GRAPH_CONFIG","paragraph profile overlay is missing approved composition or imagegen rules");
  }
  ...
  if (!within(identityPath, sharedRoot) || !within(stylePath, sharedRoot)) {
    fail("INVALID_GRAPH_CONFIG","shared identity or style reference escapes the shared skill");
  }
  return {
    runtimeConfig,
    profileOverlay,
